...
public class HUDManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI victoryText; // Texto que exibe a vitória
    ...
    void Update()
    {
        ...
        CheckVictoryCondition(); // Verifica se o jogador venceu
    ...
    {
        bool allMissionsCompleted = missions.TrueForAll(m => m.completed);

        if (allMissionsCompleted && popularity >= 100f && pollution <= 0f)
        {
    ...
    {
        vitoriaAlcancada = true; // <-- Adicionado
        Time.timeScale = 0f;
        victoryText.text = "🌎 Parabéns! Você transformou a cidade em um lugar sustentável.\n\n" +
                           "✅ Popularidade: 100%\n" +
                           "✅ Meio ambiente: 100%\n" +
                           "✅ Poluição: 0%";
        victoryPanel.SetActive(true);
        // Opcional: esconde o painel de missão completa se ainda estiver visível
    ...
    {
        if (vitoriaAlcancada) return; // <-- Impede o painel se a vitória já foi atingida
